define(["require", "exports", "./world", "./player", "./sprite"], function (require, exports, World, Player, Sprite) {
    "use strict";
    ...
    var GameRender = /** @class */ (function () {
        ...
        GameRender.prototype.importSpriteSet = function (spriteIds) {
            var obj = this;
            for (var i in spriteIds) {
                var imgId = spriteIds[i];
        ...
        GameRender.prototype.cacheSprite = function (imgId, charset) {
            ...
            for (var y = 0; y < charsetData.length; y++) {
                var canvas = document.createElement('canvas'), ctx = canvas.getContext("2d"), offsetx = 0, offsety = Sprite.Sprite.height * y, splicedImg = new Image();
        ...
        GameRender.prototype.cacheMapObject = function (itemSet, itemType) {
            ...
            var canvas = $('<canvas>')[0], ctx = canvas.getContext('2d'), data = World.MapItem.getItemData(itemType), width = (data.width * 50) * data.multiplier, height = (data.height * 50) * data.multiplier, splicedImg = new Image();
            canvas.width = width;
        ...
        GameRender.prototype.refresh = function (gameData) {
            var obj = this;
            if (!obj.rendering) {
                obj.rendering = true;
                var start = new Date(), duration;
                obj.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
                obj.moveViewCenter(gameData.playerX, gameData.playerY);
                obj.renderView(gameData, gameData.playerX, gameData.playerY);
                obj.drawMapObjects(gameData, gameData.playerX, gameData.playerY);
                obj.rendering = false;
        ...
        GameRender.prototype.drawMapObjects = function (gameDataLight, x, y) {
            ...
            for (var o in gameDataLight.drawObjects) {
                if (gameDataLight.drawObjects[o].mapObjectType == World.MapObjectType.sprite) {
                    obj.drawSprite(gameDataLight.drawObjects[o]);
                }
                else if (gameDataLight.drawObjects[o].mapObjectType == World.MapObjectType.item) {
                    obj.drawMapItem(gameDataLight.drawObjects[o]);
                }
            }
        };
        GameRender.prototype.drawSprite = function (sprite) {
            var obj = this, block = World.Map.getBlock(obj.mapRender.map, sprite.x, sprite.y), offsetX = sprite.x - (Sprite.Sprite.width / 2.2), offsetY = sprite.y - (.9 * Sprite.Sprite.height), centeredX = offsetX - obj.view.startX, centeredY = offsetY - obj.view.startY, imgSet = obj.getCachedImgSet(sprite.imgId), img = imgSet.value[sprite.currAnim][0];
            if (block.type === World.TerrainType.shallow || block.type === World.TerrainType.ocean) {
                var bordersLand = World.Map.blockBorders(obj.mapRender.map, block, [World.TerrainType.beach, World.TerrainType.dirt, World.TerrainType.grass, World.TerrainType.rock]), bordersOcean = World.Map.blockBorders(obj.mapRender.map, block, [World.TerrainType.ocean]);
        ...
        GameRender.prototype.drawMapItem = function (mObj) {
            ...
            offsetX = mObj.x - (mObj.width / 2), offsetY = mObj.y - (mObj.height / 2), centeredX = offsetX - obj.view.startX, centeredY = offsetY - obj.view.startY, withinX = centeredX >= -mObj.width && centeredX + mObj.width <= obj.canvas.width + mObj.width, withinY = centeredY >= -mObj.height && centeredY + mObj.height <= obj.canvas.height + mObj.height, imgSet = obj.getCachedImgSet(mObj.imgId), img = imgSet.value[0][0];
            if (withinX && withinY)
                obj.ctx.drawImage(img, 0, 0, img.width, img.height, centeredX, centeredY, img.width, img.height);
        };
        return GameRender;
    }());
