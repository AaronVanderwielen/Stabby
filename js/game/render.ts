...
interface IRender {
    draw: (ctx: CanvasRenderingContext2D, view: ViewPort, imgCache: Array<CachedImageData>) => void;
}
...
export class GameRender {
    ...
    loadAssets(callback: Function) {
        ...
        function handleAssetLoaded(event: any) {
    ...
    cacheSprite(imgId: string, charset: HTMLImageElement) {
        ...
        for (var y = 0; y < charsetData.length; y++) {
            var canvas = document.createElement('canvas'),
                ...
                splicedImg: HTMLImageElement = new Image();
    ...
    cacheMapObject(itemSet: HTMLImageElement, itemType: World.ItemType) {
        ...
        var obj = this,
            imgId = World.MapItem.mapItemKey(itemType);
        ...
        var canvas = <HTMLCanvasElement>$('<canvas>')[0],
            ctx = canvas.getContext('2d'),
            data = World.MapItem.getItemData(itemType),
    ...
    refresh(gameData: Game.GameDataLight) {
        var obj = this;
    ...
    drawMapObjects(gameDataLight: Game.GameDataLight, x: number, y: number) {
        ...
        gameDataLight.drawObjects = _.sortBy(gameDataLight.drawObjects, function (o: World.IMapObject) {
            return o.y;
        });
        // sort by z index
        ...
        for (var o in gameDataLight.drawObjects) {
            if (gameDataLight.drawObjects[o].mapObjectType == World.MapObjectType.sprite) {
                ...
                obj.drawMapItem(<World.MapItem>gameDataLight.drawObjects[o]);
            }
        }
    }
    ...
    drawSprite(sprite: Sprite.Sprite) {
        var obj = this,
            block = World.Map.getBlock(obj.mapRender.map, sprite.x, sprite.y),
            ...
            imgSet = obj.getCachedImgSet(sprite.imgId),
            img: HTMLImageElement = imgSet.value[sprite.currAnim][0];

        if (block.type === World.TerrainType.shallow || block.type === World.TerrainType.ocean) {
            var bordersLand = World.Map.blockBorders(obj.mapRender.map, block, [World.TerrainType.beach, World.TerrainType.dirt, World.TerrainType.grass, World.TerrainType.rock]),
                bordersOcean = World.Map.blockBorders(obj.mapRender.map, block, [World.TerrainType.ocean]);
    ...
    drawMapItem(mObj: World.MapItem) {
        var obj = this,
            //offsetX = (mObj.x - obj.view.startX) - (mObj.width / 2),
            //offsetY = (mObj.y - obj.view.startY) - (mObj.height / 2),
            offsetX = mObj.x - (mObj.width / 2),
            offsetY = mObj.y - (mObj.height / 2),
            centeredX = offsetX - obj.view.startX,
            centeredY = offsetY - obj.view.startY,
            withinX = centeredX >= -mObj.width && centeredX + mObj.width <= obj.canvas.width + mObj.width,
            withinY = centeredY >= -mObj.height && centeredY + mObj.height <= obj.canvas.height + mObj.height,
